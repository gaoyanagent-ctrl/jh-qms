  ...
  'zh-CN': {
    translation: {
      ...
      mdm: {
        ...
        modelImport: { action:'导入模型字典',title:'模型数据字典 Excel 预检',guideTitle:'一个工作簿可以整理多个系统的模型',guideDescription:'请先下载标准模板，填写模型、字段、枚举和关联定义。上传只做校验和差异计算，不会修改当前模型。',downloadTemplate:'下载模型字典模板',dropTitle:'点击或拖拽模型数据字典到这里',dropHelp:'支持 .xlsx、.xls，文件不超过 20 MB',precheck:'开始预检',noWriteHint:'预检不会写入数据库',valid:'预检通过',invalid:'预检发现问题',readyDescription:'结构有效，可以在后续确认步骤生成模型草稿。',fixDescription:'请根据 Sheet、行号和字段定位问题，修正后重新上传。',modelCount:'模型数',fieldCount:'字段数',createCount:'新增模型',updateCount:'更新模型',changeType:'模型变化',fieldChanges:'字段变化',issueCount:'问题数',issues:'需要修正的问题',sheet:'Sheet',row:'Excel 行',field:'模型/字段',problem:'问题',added:'新增 {{count}}',updated:'修改 {{count}}',unchanged:'不变 {{count}}',change:{CREATE:'新增',UPDATE:'更新'} },
        records: { title: '主数据工作台', description: '表格和表单共用模型定义、校验与版本保存管道。' },
        sections: { basic: '基本信息' },
        actions: { openWorkspace: '打开数据工作台', createModel: '新建模型', designModel: '模型设计', versionHistory: '版本历史', excelPaste: 'Excel 粘贴导入', excelFileImport: 'Excel 文件导入', submitRecord:'提交', approveRecord:'批准', rejectRecord:'驳回', deactivateRecord:'停用' },
        approval: { settingsTitle:'数据审批设置', requiredLabel:'记录生效需要审批', requiredHelp:'开启后，业务数据草稿提交进入“待审批”；关闭后提交即直接生效。此设置随模型版本发布。', roleLabel:'数据审批角色', roleHelp:'该角色下的用户会收到业务数据审批待办。', currentStatus:'当前状态由审批流程控制', comment:'处理意见', commentPlaceholder:'填写本次操作的说明（驳回时必填）', history:'审批履历', versionHistory:'数据版本履历', noHistory:'尚无审批记录', task:{description:'模型发布与主数据记录的真实待办、已办和我的申请。',businessCode:'业务编码',recordName:'主数据名称',model:'主数据模型',requester:'申请人',type:'任务类型',typeMODEL:'模型发布',typeRECORD:'数据审批',object:'审批对象'}, action:{SUBMIT:'提交',APPROVE:'批准',REJECT:'驳回',DEACTIVATE:'停用'} },
        modelApproval:{settingsTitle:'模型发布审批',settingsHelp:'模型结构、字段和布局保存后，必须由指定角色审批；通过后才生成不可变模型版本并对业务数据开放。',roleLabel:'模型发布审批角色',submit:'提交发布审批',submitted:'模型已提交发布审批',confirmSubmit:'确认提交 v{{version}} 的模型发布审批？',pending:'模型版本 v{{version}} 正在等待发布审批，审批期间不可修改。',targetVersion:'目标版本'},
        validation:{title:'关联校验规则',help:'用业务字段配置跨模型存在性和状态校验，不需要编辑 JSON。多条匹配条件按 AND 执行，保存始终进行最终校验。',add:'新增规则',save:'保存规则',saved:'校验规则已保存',ruleName:'规则名称',currentField:'当前表单字段',targetModel:'目标模型',targetField:'目标匹配字段',timing:'校验时机',strategy:'异常策略',message:'提示文案',messagePlaceholder:'例如：物料编码未在物料主数据维护，请先维护',enabled:'启用',conditions:'匹配条件',conditionsHelp:'每一行表示“当前字段或固定值 = 目标字段”，所有行必须同时满足。',sourceField:'当前字段',constant:'固定值',addCondition:'增加 AND 条件',dialogTitle:'配置关联校验',deleteConfirm:'确认删除这条校验规则？',trigger:{SAVE:'保存前校验',BLUR:'字段失焦并保存前校验'},severity:{BLOCK:'阻断保存',WARNING:'警告但允许保存'}},
        grid:{saveAll:'保存全部修改',deleteSelected:'删除所选草稿',discard:'放弃修改',discardConfirm:'确认放弃 {{count}} 条记录的未保存修改？',pending:'有 {{count}} 条记录包含未保存修改，请保存或放弃后再离开。',noChanges:'当前没有待保存修改',noSelection:'请先选择需要删除的记录',deleteStateError:'只能删除草稿或已驳回记录',deleteConfirm:'确认删除所选 {{count}} 条草稿？此操作不可撤销。',staged:'修改已加入待保存队列',saved:'已保存 {{count}} 条修改',deleted:'所选草稿已删除'},
        import: { title:'Excel 文件导入', guideTitle:'使用当前模型模板导入', guideDescription:'先下载模板填写数据，上传后系统会逐行预检查。全部通过后才会生成草稿记录。', downloadTemplate:'下载模板', dropFile:'点击或拖拽 Excel 文件到这里', fileHint:'支持 .xlsx、.xls，文件不超过 20 MB；即时预检查最多 1000 行，后台预检查最多 1 万行', precheck:'即时预检查', asyncPrecheck:'后台预检查（最多1万行）', queued:'文件已进入后台预检查队列，可在任务列表查看进度', confirm:'确认导入 {{count}} 条', precheckPassed:'{{count}} 行全部校验通过，可以确认导入', precheckFailed:'{{count}} 行中存在错误，请修改文件后重新上传', excelRow:'Excel 行', result:'结果', passed:'通过', failed:'错误', problems:'问题', history:'最近导入任务', fileName:'文件名', rowSummary:'通过/总行数', operator:'操作人', createdAt:'创建时间', viewResult:'查看', downloadSource:'原文件', downloadResult:'结果文件', status:{QUEUED:'排队中',VALIDATING:'预检查中',FAILED:'处理失败',PRECHECK_FAILED:'预检查失败',READY:'待确认',COMMITTING:'导入中',COMMITTED:'已导入'} },
        search: { keyword: '关键词', placeholder: '搜索业务编码或名称' },
        feedback: { loadFailed: '主数据加载失败，请重试。', loadSchemaFailed: '模型定义加载失败。' },
        reference: { search: '搜索 {{model}} 主数据', incomplete: 'Reference 配置不完整', empty: '无匹配主数据', configure:'配置关联', notConfigured:'尚未配置', dialogTitle:'Reference 关联配置', dialogHelp:'选择数据来源以及表单中保存和显示的字段。', targetModel:'关联模型', valueField:'保存值字段', valueFieldHelp:'保存到当前记录中的稳定唯一值，通常选择业务编码。', displayField:'显示字段', displayFieldHelp:'用户在下拉选项中看到的名称字段。', statusField:'状态字段', statusFieldHelp:'可选；用于限制哪些主数据可被引用。', allowedStatuses:'允许的状态', allowedStatusesHelp:'输入状态后按 Enter，可配置多个状态。' },
        ...
        status: { PUBLISHED: '已发布', DRAFT: '草稿', PENDING_APPROVAL:'待审批', PENDING:'待审批', APPROVED:'已批准', REJECTED:'已驳回', ACTIVE: '生效', INACTIVE: '停用' },
        changeReason: { create: '创建主数据', edit: '编辑主数据', gridEdit:'Grid/表单统一编辑' }
      },
      'mdm-material': { title: '物料主数据' },
      qms: {
        inspectionStandard: {
          title: '检验标准', empty: '尚未生成检验标准', emptyHelp: '仅已确认且属于检验范围的质量特性会进入草稿。', generate: '生成草稿', generated: '检验标准草稿已生成', sync: '同步质量特性', createVersion: '创建新版本', releasedHelp: '当前版本已发布并保持只读。如需纳入新增质量特性，请创建新版本。', synchronized: '检验清单已与最新确认状态同步', syncHelp: '质量特性确认状态发生变化后，请同步清单。已填写的检验方法、抽样和备注会保留。', save: '保存草稿', saved: '草稿已保存', back: '返回', version: '版本 {{version}}', reactionPlan: '异常反应计划', submit: '提交审批', approve: '批准', reject: '驳回', release: '发布', actionComment: '审批意见', actionSuccess: '操作已完成', documentStatus: '文件状态', approvalStatus: '审批状态', selfApprovalBlocked: '提交人不能审批自己的申请，请使用其他审批账号操作。', readOnly: '当前状态为只读，只有草稿或驳回状态可以编辑。', history: '审批履历', actor: '操作人 #{{id}}', noHistory: '尚无审批记录', columns: { sequence: '序号', category: '类别', item: '检验项目', requirement: '要求/规格', special: '特殊特性', batchSampling: '批次抽样', batchMethod: '批次方法', annualSampling: '年度抽样', annualMethod: '年度方法', remark: '备注' }
        },
        validationPlan: {
  ...
  'en-US': {
    translation: {
      ...
      menu: {
        ...
        mdmModelDesigner: 'Master Data Model Designer',
        mdmRecords: 'Master Data Workspace',
      ...
      mdm: {
        ...
        modelImport: { action:'Import model dictionary',title:'Model Dictionary Excel Precheck',guideTitle:'Organize multiple source-system models in one workbook',guideDescription:'Download the standard template and complete model, field, enum, and reference sheets. Uploading only validates and calculates differences; it does not change models.',downloadTemplate:'Download dictionary template',dropTitle:'Click or drag a model dictionary workbook here',dropHelp:'Supports .xlsx/.xls up to 20 MB',precheck:'Run precheck',noWriteHint:'Precheck does not write to the database',valid:'Precheck passed',invalid:'Precheck found problems',readyDescription:'The structure is valid and can be converted into model drafts in the confirmation step.',fixDescription:'Use the sheet, row, and field columns to correct the workbook and upload it again.',modelCount:'Models',fieldCount:'Fields',createCount:'New models',updateCount:'Updated models',changeType:'Model change',fieldChanges:'Field changes',issueCount:'Issues',issues:'Problems to fix',sheet:'Sheet',row:'Excel row',field:'Model / field',problem:'Problem',added:'Added {{count}}',updated:'Changed {{count}}',unchanged:'Unchanged {{count}}',change:{CREATE:'Create',UPDATE:'Update'} },
        records: { title: 'Master Data Workspace', description: 'Grid and form share one model, validation, and version pipeline.' },
        sections: { basic: 'Basic Information' }, actions: { openWorkspace: 'Open Data Workspace', createModel: 'Create Model', designModel: 'Model Designer', versionHistory: 'Version History', excelPaste: 'Paste from Excel', excelFileImport:'Import Excel File', submitRecord:'Submit', approveRecord:'Approve', rejectRecord:'Reject', deactivateRecord:'Deactivate' },
        approval: { settingsTitle:'Data approval settings', requiredLabel:'Require approval before activation', requiredHelp:'When enabled, submitted data drafts wait for approval. When disabled, submission activates the record immediately. This setting is published with the model version.', roleLabel:'Data approver role', roleHelp:'Users in this role receive master-data approval tasks.', currentStatus:'Current status is controlled by the approval workflow', comment:'Comment', commentPlaceholder:'Describe this action (required when rejecting)', history:'Approval history', versionHistory:'Data version history', noHistory:'No approval actions yet', task:{description:'Live model-publication and master-data tasks, completed decisions, and requests started by you.',businessCode:'Business code',recordName:'Master data name',model:'Master data model',requester:'Requester',type:'Task type',typeMODEL:'Model publication',typeRECORD:'Data approval',object:'Approval object'}, action:{SUBMIT:'Submitted',APPROVE:'Approved',REJECT:'Rejected',DEACTIVATE:'Deactivated'} },
        modelApproval:{settingsTitle:'Model publication approval',settingsHelp:'After the model structure, fields, and layout are saved, the selected role must approve them before an immutable version is published.',roleLabel:'Model publication approver role',submit:'Submit for publication approval',submitted:'Model submitted for publication approval',confirmSubmit:'Submit model version v{{version}} for publication approval?',pending:'Model version v{{version}} is awaiting publication approval and cannot be edited.',targetVersion:'Target version'},
        validation:{title:'Reference validation rules',help:'Configure cross-model existence and status checks with business fields instead of JSON. Conditions use AND and save always performs final validation.',add:'Add rule',save:'Save rules',saved:'Validation rules saved',ruleName:'Rule name',currentField:'Current form field',targetModel:'Target model',targetField:'Target field',timing:'Validation timing',strategy:'Failure strategy',message:'User message',messagePlaceholder:'Explain what is wrong and how to correct it',enabled:'Enabled',conditions:'Match conditions',conditionsHelp:'Each row compares a current field or constant to a target field. Every row must match.',sourceField:'Current field',constant:'Constant',addCondition:'Add AND condition',dialogTitle:'Configure reference validation',deleteConfirm:'Delete this validation rule?',trigger:{SAVE:'Before save',BLUR:'On blur and before save'},severity:{BLOCK:'Block save',WARNING:'Warn and allow save'}},
        grid:{saveAll:'Save all changes',deleteSelected:'Delete selected drafts',discard:'Discard changes',discardConfirm:'Discard unsaved changes for {{count}} records?',pending:'{{count}} records contain unsaved changes. Save or discard them before leaving.',noChanges:'There are no unsaved changes',noSelection:'Select records to delete first',deleteStateError:'Only draft or rejected records can be deleted',deleteConfirm:'Delete {{count}} selected drafts? This cannot be undone.',staged:'Changes added to the pending save queue',saved:'Saved {{count}} changes',deleted:'Selected drafts deleted'},
        import: { title:'Import Excel File', guideTitle:'Import with the current model template', guideDescription:'Download and complete the template first. Every row is validated before draft records can be created.', downloadTemplate:'Download template', dropFile:'Click or drag an Excel file here', fileHint:'Supports .xlsx/.xls up to 20 MB; immediate precheck supports 1,000 rows and background precheck supports 10,000', precheck:'Immediate precheck', asyncPrecheck:'Background precheck (10k)', queued:'The workbook was queued for background precheck', confirm:'Import {{count}} records', precheckPassed:'All {{count}} rows passed validation', precheckFailed:'Some of the {{count}} rows contain errors. Correct the file and upload it again.', excelRow:'Excel row', result:'Result', passed:'Passed', failed:'Error', problems:'Problems', history:'Recent import tasks', fileName:'File', rowSummary:'Valid/total', operator:'Operator', createdAt:'Created at', viewResult:'View', downloadSource:'Source', downloadResult:'Report', status:{QUEUED:'Queued',VALIDATING:'Validating',FAILED:'Failed',PRECHECK_FAILED:'Precheck failed',READY:'Ready',COMMITTING:'Importing',COMMITTED:'Committed'} },
        ...
        reference: { search: 'Search {{model}} master data', incomplete: 'Reference configuration is incomplete', empty: 'No matching master data', configure:'Configure reference', notConfigured:'Not configured', dialogTitle:'Reference configuration', dialogHelp:'Choose the data source and the fields stored and displayed in the form.', targetModel:'Target model', valueField:'Stored value field', valueFieldHelp:'Stable unique value stored in the current record, usually the business code.', displayField:'Display field', displayFieldHelp:'Human-readable field shown in reference options.', statusField:'Status field', statusFieldHelp:'Optional field used to limit which master records can be referenced.', allowedStatuses:'Allowed statuses', allowedStatusesHelp:'Enter a status and press Enter. Multiple statuses are supported.' },
        ...
        status: { PUBLISHED: 'Published', DRAFT: 'Draft', PENDING_APPROVAL:'Pending approval', PENDING:'Pending approval', APPROVED:'Approved', REJECTED:'Rejected', ACTIVE: 'Active', INACTIVE: 'Inactive' }, changeReason: { create: 'Create master data', edit: 'Edit master data', gridEdit:'Unified Grid/form edit' }
      },
      'mdm-material': { title: 'Material Master' },
      qms: {
        inspectionStandard: {
          title: 'Inspection Standard', empty: 'No inspection standard yet', emptyHelp: 'Only confirmed characteristics within inspection scope are included.', generate: 'Generate draft', generated: 'Draft generated', sync: 'Sync characteristics', createVersion: 'Create new version', releasedHelp: 'The released version remains read-only. Create a new version to include newly confirmed characteristics.', synchronized: 'Inspection items synchronized with the latest confirmations', syncHelp: 'Sync after changing characteristic confirmation states. Existing inspection methods, sampling settings, and remarks are preserved.', save: 'Save draft', saved: 'Draft saved', back: 'Back', version: 'Version {{version}}', reactionPlan: 'Exception reaction plan', submit: 'Submit approval', approve: 'Approve', reject: 'Reject', release: 'Release', actionComment: 'Decision comment', actionSuccess: 'Action completed', documentStatus: 'Document status', approvalStatus: 'Approval status', selfApprovalBlocked: 'Submitters cannot approve their own requests. Use another approver account.', readOnly: 'This controlled state is read-only. Only draft or rejected documents can be edited.', history: 'Approval history', actor: 'Actor #{{id}}', noHistory: 'No approval history', columns: { sequence: 'No.', category: 'Category', item: 'Inspection item', requirement: 'Requirement', special: 'Special', batchSampling: 'Batch sampling', batchMethod: 'Batch method', annualSampling: 'Annual sampling', annualMethod: 'Annual method', remark: 'Remark' }
        },
        validationPlan: {
