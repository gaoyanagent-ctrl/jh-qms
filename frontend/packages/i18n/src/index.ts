  ...
  'zh-CN': {
    translation: {
      ...
      mdm: {
        ...
        import: { title:'Excel 文件导入', guideTitle:'使用当前模型模板导入', guideDescription:'先下载模板填写数据，上传后系统会逐行预检查。全部通过后才会生成草稿记录。', downloadTemplate:'下载模板', dropFile:'点击或拖拽 Excel 文件到这里', fileHint:'支持 .xlsx、.xls，同步导入最多 1000 行', precheck:'上传并预检查', confirm:'确认导入 {{count}} 条', precheckPassed:'{{count}} 行全部校验通过，可以确认导入', precheckFailed:'{{count}} 行中存在错误，请修改文件后重新上传', excelRow:'Excel 行', result:'结果', passed:'通过', failed:'错误', problems:'问题', history:'最近导入任务', fileName:'文件名', rowSummary:'通过/总行数', operator:'操作人', createdAt:'创建时间', viewResult:'查看结果', status:{PRECHECK_FAILED:'预检查失败',READY:'待确认',COMMITTING:'导入中',COMMITTED:'已导入'} },
  ...
  'en-US': {
    translation: {
      ...
      mdm: {
        ...
        import: { title:'Import Excel File', guideTitle:'Import with the current model template', guideDescription:'Download and complete the template first. Every row is validated before draft records can be created.', downloadTemplate:'Download template', dropFile:'Click or drag an Excel file here', fileHint:'Supports .xlsx and .xls, up to 1,000 rows per synchronous import', precheck:'Upload and precheck', confirm:'Import {{count}} records', precheckPassed:'All {{count}} rows passed validation', precheckFailed:'Some of the {{count}} rows contain errors. Correct the file and upload it again.', excelRow:'Excel row', result:'Result', passed:'Passed', failed:'Error', problems:'Problems', history:'Recent import tasks', fileName:'File', rowSummary:'Valid/total', operator:'Operator', createdAt:'Created at', viewResult:'View result', status:{PRECHECK_FAILED:'Precheck failed',READY:'Ready',COMMITTING:'Importing',COMMITTED:'Committed'} },
