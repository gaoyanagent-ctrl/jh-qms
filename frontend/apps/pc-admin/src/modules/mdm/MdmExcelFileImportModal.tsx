...
import { useCommitMdmImport, useMdmImportErrors, useMdmImportTasks, usePreviewMdmImport, useQueueMdmImport } from './hooks';
...
export const MdmExcelFileImportModal=({open,onClose,model}:{open:boolean;onClose:()=>void;model:MdmModel})=>{
  ...
  const queue=useQueueMdmImport(model.code);
  ...
  return <Modal open={open} onCancel={close} title={t('mdm.import.title')} width={980} destroyOnHidden footer={<Space><Button onClick={close}>{t('common.actions.cancel')}</Button><Button type="primary" disabled={!preview.data?.validation.valid||preview.data.status!=='READY'} loading={commit.isPending} onClick={()=>preview.data&&commit.mutate(preview.data.taskId)}>{t('mdm.import.confirm',{count:preview.data?.validation.total??0})}</Button></Space>}>
    <Space direction="vertical" size={16} style={{width:'100%'}}>
      ...
      <Space.Compact block><Button style={{width:'50%'}} disabled={!file} loading={preview.isPending} onClick={()=>file&&preview.mutate(file)}>{t('mdm.import.precheck')}</Button><Button style={{width:'50%'}} disabled={!file} loading={queue.isPending} onClick={()=>file&&queue.mutate(file)}>{t('mdm.import.asyncPrecheck')}</Button></Space.Compact>
      {queue.isSuccess&&<Alert type="success" showIcon message={t('mdm.import.queued')}/>}
      {(downloadError||preview.isError||queue.isError||commit.isError)&&<Alert type="error" showIcon message={downloadError??preview.error?.message??queue.error?.message??commit.error?.message}/>}
      ...
      <Table loading={tasks.isLoading} size="small" rowKey="id" pagination={{pageSize:5}} dataSource={tasks.data??[]} columns={[{title:t('mdm.import.fileName'),dataIndex:'fileName',render:(value:string,task)=><Space direction="vertical" size={0}><span>{value}</span>{task.errorMessage&&<Typography.Text type="danger">{task.errorMessage}</Typography.Text>}</Space>},{title:t('common.fields.status'),dataIndex:'status',width:130,render:(status:string)=><Tag color={status==='COMMITTED'?'success':status==='READY'?'processing':['FAILED','PRECHECK_FAILED'].includes(status)?'error':'warning'}>{t(`mdm.import.status.${status}`)}</Tag>},{title:t('mdm.import.rowSummary'),width:110,render:(_,task)=><span>{task.validRows}/{task.totalRows}</span>},{title:t('mdm.import.operator'),dataIndex:'createdByName',width:110},{title:t('mdm.import.createdAt'),dataIndex:'createdAt',width:145,render:(value:string)=>dayjs(value).format('YYYY-MM-DD HH:mm')},{title:t('common.fields.actions'),width:230,render:(_,task)=><Space size={0}><Button type="link" size="small" disabled={['QUEUED','VALIDATING','FAILED'].includes(task.status)} onClick={()=>setSelectedTaskId(task.id)}>{t('mdm.import.viewResult')}</Button><Button type="link" size="small" disabled={!task.sourceFileAvailable} onClick={()=>downloadArtifact(task.id,task.fileName,'source')}>{t('mdm.import.downloadSource')}</Button><Button type="link" size="small" disabled={['QUEUED','VALIDATING','FAILED'].includes(task.status)} onClick={()=>downloadArtifact(task.id,task.fileName,'result')}>{t('mdm.import.downloadResult')}</Button></Space>}]}/>
    </Space>
  </Modal>;
};
