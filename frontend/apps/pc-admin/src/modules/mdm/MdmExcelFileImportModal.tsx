...
export const MdmExcelFileImportModal=({open,onClose,model}:{open:boolean;onClose:()=>void;model:MdmModel})=>{
  ...
  return <Modal open={open} onCancel={close} title={t('mdm.import.title')} width={980} destroyOnHidden footer={<Space><Button onClick={close}>{t('common.actions.cancel')}</Button><Button type="primary" disabled={!preview.data?.validation.valid||preview.data.status!=='READY'} loading={commit.isPending} onClick={()=>preview.data&&commit.mutate(preview.data.taskId)}>{t('mdm.import.confirm',{count:preview.data?.validation.total??0})}</Button></Space>}>
    <Space direction="vertical" size={16} style={{width:'100%'}}>
      ...
      <Table loading={tasks.isLoading} size="small" rowKey="id" pagination={{pageSize:5}} dataSource={tasks.data??[]} columns={[{title:t('mdm.import.fileName'),dataIndex:'fileName'},{title:t('common.fields.status'),dataIndex:'status',width:140,render:(status:string)=><Tag color={status==='COMMITTED'?'success':status==='READY'?'processing':status==='PRECHECK_FAILED'?'error':'warning'}>{t(`mdm.import.status.${status}`)}</Tag>},{title:t('mdm.import.rowSummary'),width:160,render:(_,task)=><span>{task.validRows}/{task.totalRows}</span>},{title:t('mdm.import.operator'),dataIndex:'createdByName',width:120},{title:t('mdm.import.createdAt'),dataIndex:'createdAt',width:165,render:(value:string)=>dayjs(value).format('YYYY-MM-DD HH:mm')},{title:t('common.fields.actions'),width:90,render:(_,task)=><Button type="link" size="small" onClick={()=>setSelectedTaskId(task.id)}>{t('mdm.import.viewResult')}</Button>}]}/>
    </Space>
  </Modal>;
};
