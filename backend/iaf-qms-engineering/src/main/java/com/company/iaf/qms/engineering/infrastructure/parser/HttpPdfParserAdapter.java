...
import com.company.iaf.qms.engineering.application.PdfParserPort;
...
@Component
public class HttpPdfParserAdapter implements PdfParserPort {
    private final RestClient client;

    public HttpPdfParserAdapter(RestClient.Builder builder,
    ...
    public DrawingParseResult parse(byte[] content, String fileName, String documentId, String revisionCode) {
        ...
        ParserResponse response = client.post().uri("/internal/v1/pdf/parse")
                .contentType(MediaType.MULTIPART_FORM_DATA).body(body).retrieve().body(ParserResponse.class);
        if (response == null) throw new IllegalStateException("Parser returned an empty response");
