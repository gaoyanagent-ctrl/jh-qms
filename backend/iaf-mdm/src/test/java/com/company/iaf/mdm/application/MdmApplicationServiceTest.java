...
import com.company.iaf.platform.workflow.application.ApprovalRecord;
...
class MdmApplicationServiceTest {
    @Test void submittingModelPublicationCreatesPlatformApprovalAndLocksDraft() {
        var repository=mock(MdmRepository.class);var approvals=mock(ApprovalApplicationService.class);
        var field=new MdmModels.Field(1,"code","编码","STRING",true,true,false,true,true,true,32,List.of(),null,10,null);
        var draft=new MdmModels.Model(7,"manufacturing","material","物料","MASTER",true,true,true,false,"DRAFT",1,Map.of(),List.of(field),4L,"NOT_SUBMITTED",null);
        var pending=new MdmModels.Model(7,"manufacturing","material","物料","MASTER",true,true,true,false,"PENDING_APPROVAL",1,Map.of(),List.of(field),4L,"PENDING",2L);
        when(repository.findModel(1,"material")).thenReturn(Optional.of(draft),Optional.of(draft),Optional.of(pending));
        when(repository.roleExists(1,4)).thenReturn(true);when(repository.submitModelApproval(1,9,7,2)).thenReturn(true);

        new MdmApplicationService(repository,rules(repository),approvals).submitModelPublication(1,2,9,"material","请发布");

        verify(approvals).submit(1,2,"MDM_MODEL_PUBLISH",7,9,"请发布");
        verify(repository).submitModelApproval(1,9,7,2);
    }

    @Test void approvingModelPublicationUsesPlatformApprovalThenPublishesSnapshot() {
        var repository=mock(MdmRepository.class);var approvals=mock(ApprovalApplicationService.class);
        var pending=new MdmModels.Model(7,"manufacturing","material","物料","MASTER",true,true,true,false,"PENDING_APPROVAL",1,Map.of(),List.of(),4L,"PENDING",2L);
        var published=new MdmModels.Model(7,"manufacturing","material","物料","MASTER",true,true,true,false,"PUBLISHED",2,Map.of(),List.of(),4L,"APPROVED",2L);
        when(repository.findModel(1,"material")).thenReturn(Optional.of(pending),Optional.of(published));when(repository.userHasRole(1,10,4)).thenReturn(true);
        when(approvals.approve(1,2,"MDM_MODEL_PUBLISH",7,10,"同意")).thenReturn(new ApprovalRecord(3,"MDM_MODEL_PUBLISH",7, ApprovalStatus.APPROVED,9, LocalDateTime.now(),10L,LocalDateTime.now(),"同意",List.of()));

        new MdmApplicationService(repository,rules(repository),approvals).approveModelPublication(1,10,"material","同意");

        verify(repository).publishModel(1,10,pending);
    }
    @Test void userOutsideConfiguredRoleCannotApprove() {
        ...
        assertThatThrownBy(()->service(repository).approve(1,9,"material",recordId,"同意"))
    ...
    @Test void approvalRequiredRecordIsSubmittedAndAudited() {
        ...
        service(repository).submit(1,9,"material",recordId,"请审批");
    ...
    @Test void publishedModelCanBeSavedAsNextDraft() {
        ...
        var request=new MdmDtos.SaveModelDraftRequest(true,4L,List.of(field),uiSchema);
        when(repository.roleExists(1,3)).thenReturn(true);
        when(repository.roleExists(1,4)).thenReturn(true);

        service(repository).saveDraft(1,9,"material",request);

        verify(repository).replaceDraft(1,9,7,true,4,List.of(field),uiSchema);
    }
    ...
    @Test void returnsImmutableHistoryForRecordInRequestedModel() {
        ...
        service(repository).recordVersions(1,"material",recordId);
    ...
    @Test void batchPrecheckReturnsRowLevelDuplicateAndModelErrors() {
        ...
        var result=service(repository).validateBatch(1,"material",new MdmDtos.BatchRecordRequest(List.of(first,second)));
