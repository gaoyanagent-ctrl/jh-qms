...
class MdmControllerTest {
    ...
    @Test void resolvesExplicitModelAndPagingParameterNames() throws Exception {
        ...
        var dictionary=mock(MdmModelDictionaryExcelService.class);
        when(dictionary.template()).thenReturn(new byte[]{3,4});
        when(dictionary.preview(eq(1L),any())).thenReturn(new com.company.iaf.mdm.interfaces.dto.MdmDtos.ModelDictionaryPreview(true,0,0,0,0,List.of(),List.of()));
        var mvc=MockMvcBuilders.standaloneSetup(new MdmController(service,excel,dictionary)).build();
        ...
        mvc.perform(get("/api/mdm/models/dictionary-import-template")).andExpect(status().isOk());
        mvc.perform(multipart("/api/mdm/models/dictionary-imports/preview").file(new MockMultipartFile("file","dictionary.xlsx","application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",new byte[]{1}))).andExpect(status().isOk());
