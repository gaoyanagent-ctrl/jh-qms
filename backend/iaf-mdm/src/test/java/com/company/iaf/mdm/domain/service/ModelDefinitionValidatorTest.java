...
class ModelDefinitionValidatorTest {
    ...
    @Test void acceptsValidDefinition() {
        var field=new MdmDtos.FieldDraft("materialType","物料类型","ENUM",true,false,false,true,true,true,32,List.of("RAW"),null,10);
        assertThat(validator.validate(List.of(field)).valid()).isTrue();
    }
    @Test void reportsDuplicateAndMissingEnumOptions() {
        var first=new MdmDtos.FieldDraft("kind","类型","ENUM",false,false,false,false,false,true,null,List.of(),null,10);
        var second=new MdmDtos.FieldDraft("kind","重复","STRING",false,false,false,false,false,true,null,List.of(),null,20);
