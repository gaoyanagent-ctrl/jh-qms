...
- `MdmModelDictionaryExcelService` generates the multi-model metadata workbook and performs read-only model/field/enum/reference validation plus tenant-model difference calculation.
...
- `MdmApplicationService`: tenant-scoped unified validation, create/update, atomic Grid batch update/delete, version snapshot, optimistic-lock and model/record approval orchestration; model publication uses the platform `ApprovalApplicationService` boundary.
