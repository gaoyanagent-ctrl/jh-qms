...
- Reference fields progressively reveal target model, stored value, display label and optional status-filter selectors; stable model/field codes are persisted instead of environment-specific row IDs.
...
- Tests:
  ...
  - `src/layouts/MainLayout.test.tsx`: menu permission hiding, QMS group title/i18n rendering, group expand/collapse controls, preference reset persistence, preference drawer sidebar save behavior, and direct shell sidebar collapse/resize backend persistence.
...
- TASK-0421 keeps a visible `修改已确认项` action on confirmed characteristics. The same labeled
  review form supports correction and reconfirmation without making the confirmed state irreversible.
