...
Nginx proxies to the JH QMS frontend container on `127.0.0.1:15174`. The frontend
container serves the SPA and proxies `/api/` to the backend over the private Compose
network. The backend is additionally bound to `127.0.0.1:18082` for host-local diagnosis.
