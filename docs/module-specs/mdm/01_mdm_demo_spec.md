...
- 模型中心支持一个多 Sheet 工作簿描述多个模型、字段、枚举和 Reference，适合汇总 ERP、MES、WMS、QMS 等系统的数据字典。
- 导入采用“模板下载 → 上传预检 → 差异预览 → 确认生成草稿 → 发布审批”，预检不得写库或影响已发布版本。
- 模型和字段使用稳定业务编码关联，不使用数据库 ID；跨模型 Reference 可以引用同一工作簿中待创建的模型。
- 差异分为模型新增/更新以及字段新增/修改/不变，Excel 缺失的已有字段默认保留，不推断删除。
- 首个切片交付模板、预检和差异预览；确认生成草稿、导出现有字典、布局及校验规则 Sheet 在后续切片完成。
